...
public class Matrice3DTL extends Matrice {
    ...
    private int n;
    private HashMap<Integer, Double> registre = new HashMap<Integer, Double>();
    ...
    private Integer hash(int i, int j) {
        return i + n * j;
    }
    ...
    public void set(int i, int j, double x) {
        ...
        registre.put(hash(i, j), x);
    }

    @Override
    public double get(int i, int j) {
        if (mauvaixIndex(i, j) || this.n <= 0) return 0.0;
        Double reg = registre.get(hash(i, j));
